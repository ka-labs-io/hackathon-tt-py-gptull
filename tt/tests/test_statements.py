...
from tt.statements import TransformContext, transform_block, transform_statement
...
class TestTransformContext:
    ...
    def test_indented_creates_child_context(self) -> None:
        ctx = TransformContext(indent_level=0)
        child = ctx.indented()
    ...
    def test_double_indented(self) -> None:
        expected_level = 2
        ctx = TransformContext().indented().indented()
...
class TestVariableDeclarations:
    def test_const_number(self) -> None:
        ...
        result = transform_statement(node, TransformContext())  # type: ignore[arg-type]
        assert result == ["x = 1"]
    ...
    def test_indented_declaration(self) -> None:
        node = _parse_first_statement("const x = 1;")
        ctx = TransformContext(indent_level=2)
...
class TestTransformBlock:
    def test_multiple_statements(self) -> None:
        ...
        result = transform_block(block_node, TransformContext(indent_level=1))  # type: ignore[arg-type]
